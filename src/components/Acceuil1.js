...
import Vid1 from '../images/omp.mp4';
import Vid2 from '../images/vertical.mp4';
...
const Acceuil1 = () => {
  return (
    <div
      ...
      name="APROPOS"
    >
              <video src={Vid1} autoplay="{true}" loop muted  className="absolute top-0 left-0 w-full h-full -z-1 hidden md:block object-cover"></video>
              <video src={Vid2} autoplay="{true}" loop muted  className="absolute top-0 left-0 w-full h-full -z-1 md:hidden block object-cover"></video>


      <div
        className=" md:px-8 px-2  bg-greenlight/75  md:w-1/3 h-1/2 sm:w-1/2 w-9/12 space-y-2 rounded-lg md:space-y-4 xl:space-y-4 mt-28 mr-20 drop-shadow-2xl   "
        data-aos="fade-right"
